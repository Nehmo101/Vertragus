...
describe('the i18n module itself', () => {
  ...
  it('translates, interpolates and pluralizes in both languages', () => {
    ...
    expect(dt('panel.agentCount', { count: 1 })).toBe('1 Agent')
    expect(dt('panel.agentCount', { count: 2 })).toBe('2 Agenten')
    expect(et('panel.agentCount', { count: 1 })).toBe('1 agent')
    expect(et('panel.agentCount', { count: 0 })).toBe('0 agents')
