 ...
 * cleanup list, folded out below the row), gear (profile editor). Play is
 * deliberately the visually loudest control in the panel; it is the one thing
 * the app exists to do. Name/count stay a sibling of the three buttons so
 * filter clicks never hit those targets.
 *
 * The row itself is a `div` inside an `li` wrapper: the cleanup list unfolds
 * as a second child below the row, and it must not inherit the row's flex
