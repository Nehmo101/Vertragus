...
describe('runHeadless lifecycle', () => {
  ...
  it('times out while command resolution is pending without spawning', async () => {
    vi.useFakeTimers()
    mocks.resolveLaunch.mockReturnValueOnce(new Promise(() => {}))
    const handle = runHeadless('claude', 'task', opts, vi.fn(), { timeoutMs: 25 })

    await vi.advanceTimersByTimeAsync(25)

    await expect(handle.done).resolves.toMatchObject({ status: 'timed_out', isError: true })
    expect(mocks.spawn).not.toHaveBeenCalled()
  })
