...
describe('PendingQuestions', () => {
  ...
  it('notifies mutation listeners on create, answer, cancel, and clear', () => {
    const questions = registry()
    let n = 0
    const off = questions.onMutate(() => n++)
    questions.create('a1', 'q?')
    expect(n).toBe(1)
    questions.answer('q1', 'yes')
    expect(n).toBe(2)
    questions.create('a1', 'again?')
    questions.create('a2', 'theirs?')
    expect(n).toBe(4)
    expect(questions.cancelForAgent('a1')).toBe(1)
    expect(n).toBe(5)
    questions.clear()
