...
export const orchestratorSystemPrompt = (
  ...
  '- Never list a shared hotspot (src/shared, src/main/ipc, src/preload, global CSS) in the expectedFiles of a FEATURE task — the validator then rejects the whole plan and collapses it to one worker. Declare those files only on the integrator.',
  ...
  '- An invalid plan is rejected and collapsed to a single fallback worker — inspect validationIssues, fix the plan and resubmit a valid multi-task plan instead of accepting the collapse.',
