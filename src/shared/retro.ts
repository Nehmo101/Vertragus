...
type RetroTask = OrcaTask & { judgeReason?: string }
...
const INFRA_FAILURE_PATTERNS = [
  /\bat capacity\b/i,
  /\b(?:provider|service|server)\b.{0,100}\b(?:capacity|kapazität|overloaded|überlastet|ausgelastet|unavailable|nicht verfügbar)\b/i,
...
function classifyRetroFailure(
  ...
  failureKind: OrcaTask['failureKind'],
  evidence: string
): RetroFailureKind {
...
function taskFailureEvidence(task: RetroTask): string {
...
function countCodeGateFindings(task: RetroTask, failureKind: RetroFailureKind): number {
 ...
 */
export function deriveModelStats(tasks: RetroTask[]): RetroModelStats[] {
