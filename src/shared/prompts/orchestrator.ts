...
export function buildOrchestratorSystemPrompt({
  ...
  return [
    ...
    '',
    'Isolation: you and every agent you start each work in a separate git worktree of this repository, on a separate vertragus/* branch. Agents therefore never see each other’s uncommitted files. When results from several agents must come together, have an agent commit its work and delegate the merge of those branches to an agent — the branches merge like any other git branches.',
    '',
    'Available roles:',
    roleLines,
    totalLine,
    '',
    'Your tools:',
    '- start_agent{role, task, model?} — start a subagent. The task must be self-contained: goal, the files or area involved, the definition of done, and how to verify it. Every agent automatically gets its own git worktree and branch; the response tells you where.',
