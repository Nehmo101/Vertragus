...
function renderAskUserToolLine(): string {
  return '- ask_user{question, ticket?} — ask the HUMAN and wait for the answer (they see it in the panel and on their phone). When to call it is the question-mode block above — that block is authoritative. If it returns answer: null with a ticket, call it again with that ticket and the unchanged question.'
}
...
function renderLoop(mode: QuestionMode): string[] {
  const stepZero =
    mode === 'thorough'
      ? [
          '0. Close the brief: before starting the team, diff the goal against acceptance criteria (observable happy path, non-goals, design-flip edge cases) and Definition of Done. Every hole is one numbered ask_user, batched. Another round only if the answer opens a new hole. If there are no holes, do not call ask_user. Never guess a product or scope decision the goal does not settle.'
        ]
      : []
  return [
...
export function buildLeadSystemPrompt({
  ...
  return [
    ...
    budgetLine,
    '',
    'Downward you have start_agent, send_to_agent, await_events, list_agents, stop_agent, read_output and inspect_agent — the same loop as the root: start agents, then loop on await_events with the returned cursor and handle every event; never idle without an open await_events and never poll list_agents. You only see and address YOUR OWN agents. You cannot start orchestrators — depth ends with you. Workers you start may start helpers (one extra level). You will not see helper events — inspect the worker. Helpers cannot nest further. You also share the workspace task board (task_create, task_update, task_list) with the root — one task per assignment, assign tasks only to yourself or your own agents, and complete only after verification.',
    '',
