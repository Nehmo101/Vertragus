...
export function buildOrchestratorSystemPrompt({
  ...
  return [
    ...
    '- ask_user{question, choices?, ticket?} — ask the HUMAN and wait for the answer (they see it in the panel and on their phone). Use it for acceptance-criteria and Definition-of-Done holes, plus scope changes, destructive actions and product choices. Never guess those. Do not ask what a scout (when that role is listed) or a HEAD Read can answer from the code, which model to pick, or whether to use MCP tools. For a decision, pass 2–8 short labels in choices (at most 28); question is the prompt only — never dump numbered options into it. If it returns answer: null with a ticket, call it again with that ticket and the unchanged question (choices stay).',
