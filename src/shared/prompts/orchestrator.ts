...
export function buildOrchestratorSystemPrompt({
  ...
  return [
    ...
    '- ask_user{question, choices?, ticket?} — ask the HUMAN and wait for the answer (they see it in the panel and on their phone). Only for decisions that are genuinely the user’s: scope changes, destructive actions, product choices. For a decision, pass 2–8 short labels in choices (at most 28); question is the prompt only — never dump numbered options into it. If it returns answer: null with a ticket, call it again with that ticket and the unchanged question (choices stay).',
