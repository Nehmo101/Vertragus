/**
 * Learning merge rules, ported from the archived repo's retro subsystem.
 *
 * The invariants that made the old store trustworthy are kept verbatim: an
 * identical insight for the same provider+model is reinforced (observations,
 * fresher evidence) instead of duplicated, and the store is bounded per
 * model+kind and globally so it never grows without limit. Pure and
 * deterministic — the clock is a parameter.
 */
import {
  MAX_LEARNINGS_PER_MODEL_KIND,
  MAX_LEARNINGS_TOTAL,
  type ModelLearning,
...
export function learningKey(
  learning: Pick<ModelLearning, 'providerId' | 'model' | 'kind' | 'insight'>
): string {
  return [
    learning.providerId,
...
export function mergeModelLearnings(
  ...
  for (const addition of additions) {
    ...
    if (current) {
      const updated: ModelLearning = {
        ...
        roleId: addition.roleId ?? current.roleId,
  ...
  for (const entry of byKey.values()) {
    const groupKey = `${entry.providerId}|${entry.model.toLowerCase()}|${entry.kind}`
  ...
  if (all.length > MAX_LEARNINGS_TOTAL) {
    all = all.sort((a, b) => b.updatedAt - a.updatedAt).slice(0, MAX_LEARNINGS_TOTAL)
  }
...
/** Top learnings for one provider/model, ready for prompt and panel surfaces. */
