 ...
 * the worktree, MCP attach and first-turn seed finish. Live xterm output is
 * visible for every phase except `waiting`; a compact titlebar status names
 * the phase so an empty terminal is not silent. The full greyhound overlay
 * appears only while waiting for a late MCP session (click-through, so a
 * leftover Cursor approval can still be answered).
 *
...
/**
 * Compact titlebar phase line while xterm is live (every boot phase except
 * `waiting`). `null` / `undefined` show nothing.
 */
