...
| C7 model/provider reseat (switch mid-run) | **core implemented** — see [`MODEL-PROVIDER-SWITCH.md`](./MODEL-PROVIDER-SWITCH.md) |
...
Git hangs must not swallow the done event: snapshot fails → an explicit
`snapshotError` accompanies the available facts; the orchestrator inspects afterwards. Sentinel DONE sets
...
Host snapshot and merge commits disable inherited GPG signing. Snapshot staging
excludes project MCP files from the bulk add, removes host-injected settings
from their index blobs, and preserves unrelated user edits and the live CLI
configuration. Linked profiles resolve the shared exclude file through Git.
An unknown or altered host credential fails the snapshot closed.

Stopping first quiesces agent processes; slow push/PR work follows. Finalization
attempts persist per run and can be inspected and explicitly retried after a
restart. Quit cancels push/gh before the shutdown ceiling and retains a retryable
failure, including an archive retry already in flight. The archive recovery/review reads journal identity and current Git
branch, SHA, dirtiness and changed-file facts, alongside integration events.
Resume can select a prior run and its integration branch. Cleanup shows disk
size, dirty state, commits ahead of the checkout and run affiliation; removal
remains explicit, without force or branch deletion.
...
(`reseat_agent`): it preserves the same identity, worktree, question and slot
across the process switch. `stop_agent` preserves files and branches, but a
later `start_agent` creates a new identity and remains bound to profile slots.

The explicit root/worker/lead reseat path is implemented, with preflight, snapshot-before-kill, token rotation and generation fencing. The host exposes effective seat and boot history; recovery restores the prior root seat. The optional in-session model command remains a design option.
...
`budget_warning` / no new starts. **No guessed token counters** now means
**CLI-recorded only**: the budget stays a wall clock, succession stays
self-declared, and the overview may show a number only when the agent's
own CLI wrote one on disk. The number never drives a host decision.

| Preset | `usageSource` | What the overview shows |
| --- | --- | --- |
| claude | `claude-jsonl` at `~/.claude/projects/<slug>/<id>.jsonl`, pinned by `--session-id` (agent id) | consumption |
| codex | `codex-rollout` under `~/.codex/sessions/YYYY/MM/DD/`, matched by recorded `session_meta.cwd` (no launch pin); last cumulative `thread_token_usage` wins | consumption |
| grok | `grok-session` under `~/.grok/sessions/<encoded cwd>/<id>/`, pinned by `--session-id`; occupancy only | labelled `context` — not consumption |
| cursor, kimi, ollama | none | nothing |

The host reads at `report_done` (awaited before the event is pushed), at
`stop_agent` (Workspace reads before the kill), and at process exit
(cached value on the event, then an async refresh). Readers live in
`src/main/providers/usage.ts` and fail soft: missing file, oversize, or
parse failure omits the field. `AgentHost.readTokenUsage` is optional; a
host without the method is a no-op.

The field rides `agent_done`, `agent_stopped`, `agent_exited`, and
`orchestrator_exited` into `events.jsonl`. `list_agents` keeps it;
`await_events` slim rows drop it. Only the overview paints it:
`TimelineApp` passes `showUsage` into `WorkspaceCard`; the compact panel
does not. Presence of `tokenUsage` is the state. The `agent_done` journal
line appends the compact count. The phone forwards the field and does not
draw it.

Design record: [`plans/token-usage-overview.md`](./plans/token-usage-overview.md).
...
- A spawn overlay of a foreign agent CLI, or a first-party coding-agent
  runtime (slots stay Claude / Cursor / Codex / Kimi / Grok / Ollama;
  the process is that CLI). Exit plan:
...
PR #17   A1–A3 + B Remote + H3 + C1 inspect_agent + C2 done facts
     ...
     └─ Phase C   C3/C4 snapshot commit + handoff package     later
            ...
            C7 model/provider reseat (needs C3 + C6)     implemented
...
Unified session chrome (`cliSurface`) from host events, not a parsed
vendor TUI. First turn held until the Vertragus MCP session exists.
Policy tiers on the native CLIs; Cursor Run Everything is the honest
Cursor path. `mcp: none` (Ollama) reports through the sentinel dialect.
Provider-claimed MCP tool timeouts (`raisedWindow`) so `await_events`
lives. Faithful argv (`needsFaithfulArgs`) so a multiline prompt does
not die in cmd.exe.
...
Do not replace the wrap with an in-process coding agent. That is Pi as
a seventh provider under another name.
...
| CLI-recorded token usage | `providers/usage.ts`, `usageSource` on presets, `tokenUsage` on `agent_done` / `agent_stopped` / `agent_exited` / `orchestrator_exited`; overview `WorkspaceCard showUsage` | **this** |
