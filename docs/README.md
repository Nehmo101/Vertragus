# Vertragus-Dokumentation

- [Aktueller Umsetzungsstand auf DEV](./IMPLEMENTATION_STATUS.md)
- [Git-Handbuch für Branches, Worktrees und Pull Requests](./GIT_WORKFLOW.md)
- [Handbuch für Nutzung, Entwicklung und Betrieb](./ORCA_STRATOR_HANDBUCH.md)
- [Orchestrator-Training-Prompts (Trainings- und Bewertungskatalog)](./ORCHESTRATOR_TRAINING_PROMPTS.md)
- [Voice-/Speech-to-Text-Plan](./VOICE_INTERFACE_PLAN.md)
- [Produkt- und Technik-Roadmap](./ORCA_STRATOR_ROADMAP.md)

Die Roadmap enthält den geprüften Ist-Stand, bestätigte Bugs, Zielarchitektur,
Pflichtfeatures, Design-Presets, Abnahmekriterien und die empfohlene PR-Reihenfolge.
