...
2. **Im Repo:** Secret `ANTHROPIC_API_KEY` anlegen
   ...
   `VERTRAGUS_RETRO_MIN_NEW` neue Retros
...
- **Analyse lokal ausführen:**
  ...
  Unter `--min-new` (Default 3, env `VERTRAGUS_RETRO_MIN_NEW`) neuen Retros wird
  übersprungen. Das Modell ist per `VERTRAGUS_RETRO_MODEL` überschreibbar
