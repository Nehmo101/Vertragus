...
|---|---|---|---|
| Claude Code | Abgedeckt. Non-yolo-Tasks erhalten das fest verdrahtete Vertragus-MCP-Tool `mcp__vertragus-sub__permission_prompt` über `--permission-prompt-tool`. Nur Toolname und Scope erscheinen remote; `tool_input` bleibt ausschließlich im Main-Prozess. | Konservativer, provider-spezifischer Prompt-Parser als Fallback. | Timeout und Shutdown sind `deny`. |
