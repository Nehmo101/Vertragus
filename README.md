...
spawn so the process is `pi` while the roster still names Claude, Cursor,
Codex, Kimi, Grok or Ollama: the preset maps onto Pi's `--provider`, the
slot's model onto `--model`. MCP attaches through `.pi/mcp.json` and the
community adapter `npm:pi-mcp-adapter`. Cursor's closest Pi backend is
`github-copilot`; Ollama has no Pi backend, so only `--model` is passed.
Takes effect on the next Play (`npm i -g --ignore-scripts
