...
> **Remediation progress (this branch):** the following findings are already fixed on `claude/scan-everything-r66a2y` — **H1** (removed the shell + added an option-injection guard on the GitHub search query, with tests), **H2** (added `assertNotVoiceWindow` sender guards to all 13 `orchestrator:*` mutators plus `mcpSave`/`gitSwitchBranch`/`githubRepoBind`/`githubRepoSearch`/`githubAuth*`/`profileGenerateForRepo`, locked in by 20 new regression tests), **L1** (pairing rate-limit now keys on the tunnel's `cf-connecting-ip` instead of the collapsed localhost address), **L4** (RemoteReadModel now evicts snapshots for removed sessions, with a test), **M3** (git poll skips the store write when `gitInfo` is unchanged), **M6/L6/L8/L9** (deleted the dead `telemetryFormatter`, `VoiceBar`, `inboxSorter`, `modelCatalogFilter` modules + tests), and **L11 partial** (removed the dead `readGithubOAuthMeta` export). All 1238 tests, typecheck, and lint pass. *L7 (canvasSlots) was intentionally kept — its `rectsOverlap` helper backs a live auto-layout overlap test, so the "dead" claim was only partly right.* The remaining findings below are not yet addressed.
>
