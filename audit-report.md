...
>
> **Fixed — code quality / tests:** **H6** (RemoteGateway path-traversal + 401/403 + `/pair` 429 tests); **L6/L8/L9** (deleted dead `telemetryFormatter`, `VoiceBar`, `inboxSorter`, `modelCatalogFilter` + tests); **L10** (`requireProfile` unifies missing-profile handling across mutators); **L11** (removed dead `readGithubOAuthMeta`).
>
> **D1 (Electron 33 EOL → 43) — APPLIED.** Upgraded `electron` 33.4.11 → 43.1.1 and `electron-builder` 25 → 26. `pnpm audit --prod` now reports **"No known vulnerabilities found"** (was 18 advisories / 4 HIGH). Verified green here: full test suite (1252), typecheck, lint, and `electron-vite build` (main + preload + renderer all bundle). Test isolation was fixed so no unit suite depends on the Electron binary anymore (via `ELECTRON_OVERRIDE_DIST_PATH` in vitest — this also closes the audit's "7 suites hard-depend on the Electron binary" test-coverage finding). **Still required before release:** a manual smoke-test of the *packaged* app (`build:win`/`build:mac`/`build:linux` + launch), since a 10-major jump can change runtime behavior that typecheck/unit-tests don't exercise, and Windows packaging (`electron-winstaller`) can't be validated on this Linux host.
>
> **Deliberately not applied (with reasons):**
> - **D2–D7:** dev/build-time-only advisories (vitest/vite/esbuild), an EOL-but-no-CVE `electron-store`/`zod`, and the MPL `web-push` license note — none warrant a blind major bump; the optional remote deps already degrade gracefully via guarded `import().catch()`.
> - **M5 (three IPC-validation styles) and A1/A2/A5 (god-class splits of Engine/AgentManager/autoPr):** genuine multi-day structural refactors with no active bug (the sender-auth and missing-profile gaps within them are fixed above). Deferred rather than rushed blind in a codebase that can't be run here.
> - **H5 remainder** (per-leaf selector migration for TitleBar/Sidebar) and **L5/L7** (dead-ish modules kept: `orchestratorTraining`'s `trainingScenarioById` and `canvasSlots`'s `rectsOverlap` are both used by live-code tests — the "dead" claims were only partly right).
>
